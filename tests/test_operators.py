...
def test_sum_distribute(xs: List[float], ys: List[float]) -> None:
    """Write a test that ensures that the sum of `xs` plus the sum of `ys`
    is the same as the sum of each element of `xs` plus each element of `ys`.
    """
    assert_close(
        minitorch.operators.sum(addLists(xs, ys)),
        add(minitorch.operators.sum(xs), minitorch.operators.sum(ys)),
    )
